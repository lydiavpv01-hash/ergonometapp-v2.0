...
def evaluaciones_guardadas():
    ...
    evaluaciones = RebaEvaluation.query.filter_by(usuario=usuario).order_by(RebaEvaluation.created_at.desc()).all()
...
@login_required
def metodos():
    return jsonify({'métodos':[{'nombre':'REBA','url':'/reba/nueva'},{'nombre':'Ley SILLA','url':'/ley-silla/nueva'},{'nombre':'LEST','url':'/lest/nueva'},{'nombre':'Apéndice I','url':'/apendice-i/nueva'},{'nombre':'Apéndice II','url':'/apendice-ii/nueva'},{'nombre':'Kuorinka','url':'/cuestionario-nordico/nueva'}]})
