(()=>{
  ...
  function controlState(){
    ...
    return {
      neck:{twisted:c('neckTwisted'),tilted:c('neckTilted')},
      trunk:{twisted:c('trunkTwisted'),tilted:c('trunkTilted')},
      legs:{condition:v('legsCondition')||'both_supported'},
      upper_arm:{shoulder_raised:c('armRaised'),abducted:c('armAbducted'),supported:c('armSupported')},
      wrist:{deviated:c('wristDeviated')},
      load:{value:Number(v('loadValue')||0),unit:v('loadUnit')||'kg',shock:c('loadShock')},
      coupling:v('coupling')||'regular',
      activity:{static:c('activityStatic'),repetitive:c('activityRepetitive'),rapid_change:c('activityRapid')}
    };
  }
  ...
  function segmentOptions(seg){
    ...
    const radio=(name,val,label,checked='')=>`<div class="option"><label><input type="radio" name="${name}" value="${val}" ${checked}> ${label}</label></div>`;
    const check=(id,label)=>`<div class="option"><label><input id="${id}" type="checkbox"> ${label}</label></div>`;
    let body='';
    if(seg==='Cuello') body=radio('direction','flexion','Flexión','checked')+radio('direction','extension','Extensión')+check('neckTwisted','Cuello torcido (+1)')+check('neckTilted','Inclinación lateral (+1)');
    else if(seg==='Tronco') body=radio('direction','flexion','Flexión','checked')+radio('direction','extension','Extensión')+radio('direction','neutral','Neutral')+check('trunkTwisted','Tronco torcido (+1)')+check('trunkTilted','Inclinación lateral (+1)');
    ...
    else if(seg==='Brazo') body=radio('direction','flexion','Flexión','checked')+radio('direction','extension','Extensión')+radio('direction','neutral','Neutral')+check('armRaised','Hombro elevado (+1)')+check('armAbducted','Brazo abducido (+1)')+check('armSupported','Brazo apoyado (-1)');
    ...
    else if(seg==='Muñeca') body=radio('direction','flexion','Flexión','checked')+radio('direction','extension','Extensión')+radio('direction','neutral','Neutral')+check('wristDeviated','Desviación / torsión de muñeca (+1)');
    else if(seg==='Carga') body=`<div class="option"><label>Carga manipulada</label><div style="display:grid;grid-template-columns:1fr 90px;gap:8px;margin-top:8px"><input id="loadValue" type="number" min="0" step="0.1" value="0" style="padding:9px;border:1px solid #dfe4dd;border-radius:7px"><select id="loadUnit" style="padding:9px;border:1px solid #dfe4dd;border-radius:7px"><option value="kg">kg</option><option value="lb">lb</option></select></div></div>${check('loadShock','Carga aplicada con golpe o fuerza brusca (+1)')}`;
    else if(seg==='Acoplamiento') body=`<div class="option"><label>Calidad del acoplamiento</label><select id="coupling" style="width:100%;margin-top:8px;padding:9px;border:1px solid #dfe4dd;border-radius:7px"><option value="good">Bueno</option><option value="regular" selected>Regular</option><option value="poor">Malo</option><option value="unacceptable">Inaceptable</option></select></div>`;
    ...
    qa('input[name="direction"]').forEach(x=>x.onchange=()=>{ try{ renderGuides(); }catch{}; calculateLive(); });
    options.querySelectorAll('input,select').forEach(x=>x.addEventListener('change',calculateLive));
  }
  ...
  function payload(){
    ...
    return {
      neck:{angle:n.angle,direction:n.direction||'flexion',twisted:s.neck.twisted,tilted:s.neck.tilted},
      trunk:{angle:t.angle,direction:t.direction||'flexion',twisted:s.trunk.twisted,tilted:s.trunk.tilted},
      legs:{condition:s.legs.condition,knee_angle:l.angle},
      upper_arm:{angle:a.angle,direction:a.direction||'flexion',shoulder_raised:s.upper_arm.shoulder_raised,abducted:s.upper_arm.abducted,supported:s.upper_arm.supported},
      forearm:{angle:f.angle},
      wrist:{angle:w.angle,direction:w.direction||'flexion',deviated:s.wrist.deviated},
      load:{value:s.load.value,unit:s.load.unit},
      shock:s.load.shock,
      coupling:s.coupling,
      activity:s.activity
    };
  ...
  function setScores(r){
    ...
    if(!r){ scoreEls.forEach(e=>e.textContent='—'); return; }
  ...
  async function saveEvaluation(){
    ...
    if(!lastResult) await calculateLive();
    ...
    try{
      const res=await fetch('/reba/nueva',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({meta,evaluation:p,result:lastResult,photos})});
      const j=await res.json();
  ...
  if(typeof render==='function'){
    const timer=setInterval(calculateLive,900);
